...
const Dashboard = () => {
  ...
  const [counts, setCounts] = useState({ projects: 0, staff: 0 });

  useEffect(() => {
    const fetch = async () => {
      const [fp, pc, sc] = await Promise.all([
      ...
      setCounts({ projects: pc.count ?? 0, staff: sc.count ?? 0 });
    };
    fetch();
  }, []);
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-3xl">
            ...
            <div className="flex flex-wrap gap-4 opacity-0 animate-fade-in" style={{ animationDelay: "0.5s" }}>
              <Link to="/projects" className="inline-flex items-center gap-2 px-6 py-3 bg-primary text-primary-foreground font-semibold rounded-md hover-glow hover:bg-primary/90 transition-colors">
                View Projects <ArrowRight size={18} />
              </Link>
              <Link to="/contact" className="inline-flex items-center gap-2 px-6 py-3 border border-border text-foreground font-semibold rounded-md hover:bg-secondary transition-colors">
                Get in Touch
              </Link>
            </div>
          </div>
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="bg-card border border-border rounded-lg p-8 md:p-12 card-hover">
              <div className="flex flex-col md:flex-row gap-8 items-center">
                <div className="w-full md:w-1/2 aspect-video rounded-md bg-secondary overflow-hidden">
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-8 text-center">
            {[
              ...
              { value: counts.staff || "—", label: "Staff" },
              { value: "∞", label: "Ideas" },
            ].map((stat) => (
              <div key={stat.label}>
