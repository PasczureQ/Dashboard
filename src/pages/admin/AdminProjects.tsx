...
const categories = [
  ...
  { value: "brand", label: "Brand / Venture" },
];

const statusesByCategory: Record<string, { value: string; label: string }[]> = {
  roblox: [
...
const AdminProjects = () => {
  ...
  const addProductImage = async (file: File) => {
    ...
    if (url) {
      setEditing(prev => prev ? { ...prev, product_images: [...(prev.product_images || []), url] } : prev);
    }
  };

  const removeProductImage = (index: number) => {
    setEditing(prev => prev ? { ...prev, product_images: (prev.product_images || []).filter((_, i) => i !== index) } : prev);
  };
        ...
        <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setEditing(null)}>
          <div className="bg-card border border-border rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-display font-bold">{editing.id ? "Edit" : "Add"} {isBrand ? "Venture" : "Project"}</h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select value={cat} onChange={(e) => { const newCat = e.target.value; setEditing({ ...editing, category: newCat, status: statusesByCategory[newCat]?.[0]?.value }); }} className="w-full px-3 py-2 bg-secondary border border-border rounded-md text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50">
                  {categories.map((c) => <option key={c.value} value={c.value}>{c.label}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-muted-foreground mb-1">Status</label>
                <select value={editing.status || availableStatuses[0].value} onChange={(e) => setEditing({ ...editing, status: e.target.value })} className="w-full px-3 py-2 bg-secondary border border-border rounded-md text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50">
              ...
              {!isBrand && (
                <div>
                  ...
                  <input value={editing.game_link || ""} onChange={(e) => setEditing({ ...editing, game_link: e.target.value })} className="w-full px-3 py-2 bg-secondary border border-border rounded-md text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="https://..." />
                </div>
              )}
              {isBrand && (
                <div>
                  <label className="block text-sm font-medium text-muted-foreground mb-1">Brand Type</label>
                  <input value={editing.brand_type || ""} onChange={(e) => setEditing({ ...editing, brand_type: e.target.value })} className="w-full px-3 py-2 bg-secondary border border-border rounded-md text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="Apparel, Electronics, etc." />
                </div>
              )}
              <div>
                <label className="block text-sm font-medium text-muted-foreground mb-1">Thumbnail / Logo</label>
                <input type="file" accept="image/*" onChange={async (e) => { const file = e.target.files?.[0]; if (file) { const url = await uploadFile(file, "projects"); if (url) setEditing({ ...editing, thumbnail_url: url }); } }} className="w-full text-sm text-muted-foreground" />
                {editing.thumbnail_url && <img src={editing.thumbnail_url} alt="" className="mt-2 h-20 rounded-md object-cover" />}
              </div>
              {isBrand && (
                <div>
                  <label className="block text-sm font-medium text-muted-foreground mb-1">Product Images</label>
                  <div className="flex flex-wrap gap-2 mb-2">
                    {(editing.product_images || []).map((img, i) => (
                      <div key={i} className="relative group">
                        <img src={img} alt="" className="h-16 w-16 rounded object-cover" />
                        <button onClick={() => removeProductImage(i)} className="absolute -top-1 -right-1 w-5 h-5 bg-destructive text-destructive-foreground rounded-full text-xs flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">×</button>
                      </div>
                    ))}
                  </div>
                  <label className="inline-flex items-center gap-2 px-3 py-2 bg-secondary border border-border rounded-md text-sm text-muted-foreground cursor-pointer hover:text-foreground transition-colors">
                    <Upload size={14} /> Add Image
                    <input type="file" accept="image/*" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) addProductImage(file); }} />
                  </label>
                </div>
              )}
